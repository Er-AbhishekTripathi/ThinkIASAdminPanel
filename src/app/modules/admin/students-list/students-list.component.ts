...
export class StudentsListComponent implements OnInit {
  ...
  searchQuery = signal('');
  loading = signal(true);
  displayedColumns: string[] = ['fullName', 'email', 'phone', 'plan', 'joinedDate', 'status', 'actions'];
  selectedStudent = signal<Student | null>(null);
  statusConfirmation = signal<Student | null>(null);
  filteredStudents = computed(() => {
    const query = this.searchQuery().trim().toLowerCase();
    if (!query) return this.students();
    return this.students().filter(student => [student.fullName, student.email, student.phone, student.type, student.role].some(value => String(value || '').toLowerCase().includes(query)));
  });
  ...
  toggleStatus(student: Student) {
    this.updatingStatus.add(student._id);
    this.userService.setStatus(student._id, student.isActive === false).subscribe({
      next: response => { this.students.update(items => items.map(item => item._id === student._id ? {...item, isActive: response.data.isActive} : item)); this.updatingStatus.delete(student._id); },
      error: error => { this.updatingStatus.delete(student._id); this.snackBar.open(error.error?.message || 'Unable to update account status', 'Close', {duration: 4000}); }
    });
  }
  openStatusConfirmation(student: Student) {
    if (!this.updatingStatus.has(student._id)) this.statusConfirmation.set(student);
  }
  closeStatusConfirmation() {
    this.statusConfirmation.set(null);
  }
  confirmStatusChange() {
    const student = this.statusConfirmation();
    if (!student) return;
  ...
  viewStudentResults(studentId: string) {
    ...
    this.router.navigate(['/prelims-results'], { 
  ...
  if (confirm(`Are you sure you want to delete ${studentName}? This action cannot be undone.`)) {
    this.userService.deleteStudent(studentId).subscribe({
      next: () => {
        this.students.set(this.students().filter(student => student._id !== studentId));
        this.snackBar.open(`Student "${studentName}" deleted successfully`, 'Close', { 
          duration: 3000 
        });
      },
      error: (error) => {
