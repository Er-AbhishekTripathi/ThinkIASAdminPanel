...
export class ModuleService {
  ...
  getAdminModules(): Observable<Module[]> {
    return this.http.get<Module[]>(`${this.apiUrl}/admin`);
  }
  ...
  // ============ ModuleTest Integration Methods ============
  ...
  getModuleTestSubmissions(moduleId: string): Observable<any> {
    return this.http.get(`${this.apiUrl}/${moduleId}/module-test-submissions`);
  }

  getModuleTestLeaderboard(moduleId: string): Observable<any> {
    return this.http.get(`${this.apiUrl}/${moduleId}/module-test-leaderboard`);
  }
  ...
  getDisplayName(item: Module): string {
    const isHindi = document.body.classList.contains('hindi');
