...
export class QuestionService {
  ...
  getQuestionsWithFilters(filters: FilterOptions): Observable<QuestionsResponse> {
    let params = new HttpParams();
    ...
    if (filters.uids && filters.uids.length > 0) {
      params = params.set('uids', filters.uids.join(','));
    }
    ...
    return this.http.get<QuestionsResponse>(`${this.apiUrl}/filter`, { params });
  }

  getTagsWithCounts(): Observable<TagWithCount[]> {
    return this.http.get<TagWithCount[]>(`${this.apiUrl}/tags/with-counts`);
  }

  getQuestionByUid(uid: string): Observable<Question> {
    return this.http.get<Question>(`${this.apiUrl}/${uid}`);
  }

  getQuestionsByTag(tagId: string, page: number = 1, limit: number = 10): Observable<QuestionsResponse> {
    const params = new HttpParams()
      .set('page', page.toString())
      .set('limit', limit.toString());

    return this.http.get<QuestionsResponse>(`${this.apiUrl}/tag/${tagId}`, { params });
  }
